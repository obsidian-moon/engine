...
class obsidian_moon_core
{
	...
	{
		include(LIBSPATH.'configs/core/configurations.php');
		// Load any configurations that we may need.
		foreach ($this->config['auto_load_class'] as $key => $value)
		{
			if (is_numeric($key)):
				$this->classes($value);
			else:
	...
	function classes($class, $alternate_name="",$p3_cname=null)
	{
		...
		if (preg_match('/^core\//', $class)) 
			$classes_location = COREPATH.'classes/'.$class_name.'.php';
		elseif (preg_match('/^third_party\//', $class)) 
			$classes_location = LIBSPATH.$class.'.php';
		else 
			$classes_location = LIBSPATH.'classes/'.$class.'.php';		
		$configs_location = LIBSPATH.'configs/'.$class.'.php';
	...
	function views($_view, $_data=NULL, $_return=FALSE)
	{
		 ...
		 */
		if (file_exists(LIBSPATH.'views/'.$_view.'.php'))
		{
			if ($_data !== NULL) extract($_data, EXTR_OVERWRITE);
			ob_start();
			include(LIBSPATH.'views/'.$_view.'.php');
