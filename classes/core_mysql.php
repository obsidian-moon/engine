...
class core_mysql {
	...
	function get($variable = '', $connection = 'connection') {
		...
		if (count($this->q_select) > 0) {
			if (count($this->q_select) > 1)
				$query = "SELECT " . implode(', ', $this->q_select);
			else
				$query = "SELECT " . $this->q_select;
			if ($this->q_from != '') {
	...
	function select($select = '*') {
		if (count($this->q_select) > 0)
			$this->q_select = array_merge($this->q_select, $this->clean_array($select));
		else
			$this->q_select = $this->clean_array($select, true);
		return $this;
	}
